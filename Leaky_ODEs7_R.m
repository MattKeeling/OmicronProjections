function [T,S,E,D,nD,U,RatioS,Age_structure, FinalState] = Leaky_ODEs6_R(MaxType, M_from_to_H, M_from_to_O, alpha, gamma, sigma, d, tau, nV_Beta, nV_Speed, HHQ, N0, ...
    V1, V2, V3, tmpTransmission_Reduction, tmpVEffI, tmpVEffS, RatioPf, WaningSpeed, WaningEfficacy, MaxTime, InitialState) %#codegen
%%
%
%  No treating REC as those that have been infected / vaccinated, but are
%  waning back to susceptibility.
%

%fprintf(1,'ODEs gamma=%g\n',gamma);
% Sets up parameters if necessary.

% Expect WaningSpeed to be [1/360 1/100 1/100 1/1500 1/300 1/30];
...
lw=length(WaningSpeed);
if lw==4
    WaningSpeed(5)=1/200; lw=5;
end
if lw==6
    WaningSpeed(7)=1/200; lw=7;
end
if lw==5
    WaningSpeed=WaningSpeed([1 2 2 3 4 4 5]);
end
WaningSpeed([2 3])=mean(WaningSpeed([2 3]));
...
[t, pop]=ode45(@Diff_3_4,[0:1:MaxTime],[InitialState],options,[L number_E_states MaxType reshape(M_from_to_H,1,[]) reshape(M_from_to_O,1,[]) ...
...
% if t(end)<MaxTime
%     fprintf('Not able to Integrate Propertly\n');
%     dP=Diff_3_4(0,[InitialState]',[L number_E_states MaxType reshape(M_from_to_H,1,[]) reshape(M_from_to_O,1,[]) ...
%     sigma' d' tau' alpha gamma N0' hhq' nV_Beta nV_Speed V1 V2 V3 reshape(Transmission_Reduction,1,[]) ...
%     reshape(VEffI,1,[]) reshape(VEffS,1,[]) WaningSpeed reshape(WaningEfficacy,1,[])]);
%     figure(1);subplot(1,1,1);
%     for i=0.5:21:length(InitialState), semilogy(i+[0 0],[1e-6 max(InitialState)],'-k'); hold on; end
%     for Vtype=1:(MaxType+1), SPV=(Vtype-1)*(9*L+4*m*L)+L; semilogy(SPV+[0 0],[1e-6 max(InitialState)],'-k','LineWidth',2); end
%     semilogy(InitialState,'xb'); hold on
%     semilogy(pop,'xc'); hold on
%     p=find(dP>0); semilogy(p,dP(p),'ko');
%     p=find(dP<0); semilogy(p,-dP(p),'rs');
%     hold off
%     figure(2);
%     for i=0.5:21:length(InitialState), semilogy(i+[0 0],[1e-6 max(InitialState)],'-k'); hold on; end
%     for Vtype=1:(MaxType+1), SPV=(Vtype-1)*(9*L+4*m*L)+L; semilogy(SPV+[0 0],[1e-6 max(InitialState)],'-k','LineWidth',2); end
%     p=find(dP<0); semilogy(p,-dP(p)./InitialState(p)','rs');
%     hold off
%     
%     pause(-1);
% end
...
% plot([0:8],pop(2,SPV+[0:8]*21+15),'-ok'); 
% set(gca,'XTick',[0:8],'XTickLabel',{'#V1','#V2','nV1','nV2','nV3','wV','WV','wR','WR'}); drawnow;
...
WaneV11=pop(SPV+5*L+[1:L]);  WaneV12=pop(SPV+6*L+[1:L]); 
WaneV21=pop(SPV+7*L+[1:L]);  WaneV22=pop(SPV+8*L+[1:L]); 
wR=pop(SPV+9*L+[1:L]);  WaneR=pop(SPV+10*L+[1:L]); 
...
%CHECKER(MaxType,dPop,pop,'WT 2');
%CHECKER2(MaxType,dPop,pop,'WT 2');
...
%CHECKER(MaxType,dPop,pop,'Alpha 1');
%CHECKER2(MaxType,dPop,pop,['nVar 1 ' num2str(Vtype)]);
...
%CHECKER(MaxType,dPop,pop,'Alpha 2');
%CHECKER2(MaxType,dPop,pop,['nVar 2 ' num2str(Vtype)]);
end
...
%CHECKER(MaxType,dPop,pop,'Vaccination');
%CHECKER2(MaxType,dPop,pop,'Vaccination');
...
%and 3 doses vaccine (if boosters act like a third dose)
% dPop(a+nVPos3)=dPop(a+nVPos3) - pop(a+nVPos3)*WaningSpeed(2);
% dPop(a+wVPos)=dPop(a+wVPos) + pop(a+nVPos3)*WaningSpeed(2);
...
%CHECKER(MaxType,dPop,pop,'Waning');
%CHECKER2(MaxType,dPop,pop,'Waning');
...
dPop(a+WaneRPos) = dPop(a+WaneRPos) - V3.*pop(a+WaneRPos)./Frac_UnBoosted;
...
% booster wR takes you to R (Vtype=3)
for Vtype=1:MaxType
    RPos=(Vtype-1)*(9*L+4*m*L)+L + (8*L+4*m*L);
    dPop(a+RPos) = dPop(a+RPos) + V3.*pop(a+wRPos).*(NumRec(a,Vtype)./sum(NumRec(a,:),2))./Frac_UnBoosted;
end
dPop(a+wRPos) = dPop(a+wRPos) - V3.*pop(a+wRPos)./Frac_UnBoosted;


% OPTION 2 BOOSTERS TAKE YOU TO R (Vtype==3).
% Vtype=3; RPos=(Vtype-1)*(9*L+4*m*L)+L + (8*L+4*m*L);
% dPop(a+RPos) = dPop(a+RPos) + V3.*(pop(a+nVPos2)+pop(a+WaneVPos)+pop(a+wVPos)+pop(a+WaneRPos)+pop(a+wRPos))./Frac_UnBoosted;
% dPop(a+nVPos2) = dPop(a+nVPos2) - V3.*pop(a+nVPos2)./Frac_UnBoosted;
% dPop(a+WaneVPos) = dPop(a+WaneVPos) - V3.*pop(a+WaneVPos)./Frac_UnBoosted;
...
%CHECKER(MaxType,dPop,pop,'Boosters');
%CHECKER2(MaxType,dPop,pop,'FinalCheck');
...
function CHECKER(MaxType, dPop,pop,txt)
% L=21;m=3; a=1:L;
% SPV=MaxType*(9*L+4*m*L)+L;
% SPV=0;
% q=find(dPop(a+SPV)<-pop(a+SPV));
% if ~isempty(q)
%     fprintf(1,'%s Fails at %d\n',txt,q);
%     pause(-1)
% end
end

function CHECKER2(MaxType, dPop,pop,txt)
% L=21;m=3; a=1:L;
% SPV=MaxType*(9*L+4*m*L)+L;
% for A=1:21
%     SdP(A)=sum(dPop(A:21:end))-sum(dPop(SPV+A+[0 1 9]*L));
% end
% q=find(abs(SdP)>1e-10);
% % fprintf(1,'%s\n',txt);
% % [dPop(1:21:end)' SdP(1)]
% if ~isempty(q)
%     fprintf(1,'%s Fails at %d\n',txt,q);
