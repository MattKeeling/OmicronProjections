...
%VoC_ODE=@Leaky_ODEs7_R_mex;   % Greater speed can be achieved using compiled mex code
VoC_ODE=@Leaky_ODEs7_R;
...
Remember_Run_Stop=Run_stop(end);
Lngth=Run_stop(end)-Run_stop(end-1);
LL=ceil(Lngth/7);
ComplianceT(length(Run_stop)-1+[1:LL])=ComplianceT(length(Run_stop));
ComplianceO(length(Run_stop)-1+[1:LL])=ComplianceO(length(Run_stop));
Run_stop(end-1+[1:LL])=Run_stop(end-1)+7*[1:LL];
ComplianceT=ComplianceT(1:length(Run_stop));
ComplianceO=ComplianceO(1:length(Run_stop));
...
Scaling=[1.0 0.924109  0.922980  0.943564  0.942524  0.946431  0.925486  0.875128];

Susceptibility2=S_Scale*Susceptibility*Scaling(Region);
...
if isempty(FS)
    ...
    [T2,S,E,D,nD2,U , Ratio, Da, FinalState]=VoC_ODE(MaxType, UK_from_toH*0 , gamma*(UK_from_toH + UK_from_toW + UK_from_toS + UK_from_toO), INC_P, Early_gamma, Susceptibility, Detection, TAU, nV_Beta, nV_Speed, 0, Region_PP(Region,:)' , V1(1,:), V2(1,:), V3(1,:), Transmission_Reduction, VEffI, VEffS, RatioPf, WaningSpeed, WaningEfficacy, 2, -1);
    
    
    %% EARLY SET-UP
    %Run Uncontrolled to 12th March (day 71)
    ...
    [T,S,E,D,nD,U , Ratio, Da, FinalState]=VoC_ODE(MaxType, gamma*UK_from_toH*Z , gamma*(UK_from_toW + UK_from_toS + UK_from_toO), INC_P, Early_gamma, Susceptibility2, Detection, TAU, nV_Beta, nV_Speed, 0, Region_PP(Region,:)' , V1(1,:), V2(1,:), V3(1,:), Transmission_Reduction, VEffI, VEffS, RatioPf, WaningSpeed, WaningEfficacy, 71-Start_Date, [FinalState]);
    T=T+Start_Date;
    
    %Self Isolation for 4 days
    [t,s,e,d,nd,u , ratio, Da, FinalState]=VoC_ODE(MaxType, gamma*UK_from_toH*Z, gamma*(UK_from_toW + UK_from_toS + UK_from_toO), INC_P, gamma, Susceptibility3, Detection, TAU, nV_Beta, nV_Speed, 0, Region_PP(Region,:)' , V1(1,:), V2(1,:), V3(1,:), Transmission_Reduction, VEffI, VEffS, RatioPf, WaningSpeed, WaningEfficacy, 4, [FinalState]);
...
for W=W_start:length(Run_stop)
    ...
    [t,s,e,d,nd,u , ratio, Da, FinalState]=VoC_ODE(MaxType, Extra_Beta*gamma*new_UK_from_toH*Z, Extra_Beta*gamma*(new_UK_from_toW + new_UK_from_toS + new_UK_from_toO), INC_P, gamma, ...
...
Distribution_Symptoms_to_ICU(1)=0;  % Don't beleive 1 day from Symp to Hosp.
...
[NDC(:,aa), nHospital, inHospital, nICU, inICU, nDeaths, INF]=ODE_to_Observables2(T, nDCH(:,aa), nE(:,aa), Region, h_factor(1),i_factor(1),d_factor(1),h_stretch(1),i_stretch(1),INC_P,...
...
[~, ~, ~, nICU, inICU, nDeaths, ~]=ODE_to_Observables2(T, nDCD(:,aa), nE(:,aa), Region, h_factor(1),i_factor(1),d_factor(1),h_stretch(1),i_stretch(1),INC_P,...
    Assumed_Delay_Reporting_Deaths, Distribution_Hosp_Time', Distribution_HospICU_Time', Distribution_ICU_Time', Distribution_Symptoms_to_Hospital, Distribution_Symptoms_to_ICU, Distribution_Hopital_to_Death,...
    Hosp_2_Death, Sympt_2_critcal, Sympt_2_hosp,  WALES_FLAG);
...
for TYPE=2:MaxType
    ...
    [~, nHospital(:,aa+LL), inHospital(:,aa+LL), nICU(:,aa+LL), inICU(:,aa+LL), nDeaths(:,aa+LL), INF(:,aa+LL)]=ODE_to_Observables2(T, nDCH(:,aa+LL), nE(:,aa+LL), Region, h_factor(TYPE),i_factor(TYPE),d_factor(TYPE),h_stretch(TYPE),i_stretch(TYPE),INC_P*nV_Speed(TYPE-1),...
        Assumed_Delay_Reporting_Deaths, Distribution_Hosp_Time', Distribution_HospICU_Time', Distribution_ICU_Time', Distribution_Symptoms_to_Hospital, Distribution_Symptoms_to_ICU, Distribution_Hopital_to_Death,...
        Hosp_2_Death, Sympt_2_critcal, S2H,  WALES_FLAG);
    [~, ~, ~, nICU(:,aa+LL), inICU(:,aa+LL), nDeaths(:,aa+LL), ~]=ODE_to_Observables2(T, nDCD(:,aa+LL), nE(:,aa+LL), Region, h_factor(TYPE),i_factor(TYPE),d_factor(TYPE),h_stretch(TYPE),i_stretch(TYPE),INC_P*nV_Speed(TYPE-1),...
        Assumed_Delay_Reporting_Deaths, Distribution_Hosp_Time', Distribution_HospICU_Time', Distribution_ICU_Time', Distribution_Symptoms_to_Hospital, Distribution_Symptoms_to_ICU, Distribution_Hopital_to_Death,...
        Hosp_2_Death, Sympt_2_critcal, S2H,  WALES_FLAG);
end
...
for Type=MaxType:-1:1
    ...
    for a=1:L
        ...
        Z=Z./sum(Ratios(:,a,:),3);
        nDCH(:,a+L*(Type-1))=nDC(:,a+L*(Type-1)).*Z;
        ...
        Z=Z./sum(Ratios(:,a,:),3);
        nDCD(:,a+L*(Type-1))=nDC(:,a+L*(Type-1)).*Z;
    end
end
